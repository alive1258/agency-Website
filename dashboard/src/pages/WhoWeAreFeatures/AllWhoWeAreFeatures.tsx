...
const AllWhoWeAreFeatures: React.FC = () => {
  ...
  const [deleteFeature] = useDeleteWhoWeAreFeatureMutation();

  const features: WhoWeAreFeature[] = data?.data || [];
  const totalPages = data?.meta?.totalPages ?? 1;
  const totalItems = data?.meta?.total ?? 0;

  const handleDelete = async (feature: WhoWeAreFeature) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete feature "${feature.title}"?`,
        icon: "warning",
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Feature "${feature.title}" has been deleted.`,
        timer: 1500,
    ...
    <div className="rounded-lg border border-gray-base bg-black-base overflow-hidden">
      <div className="overflow-x-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-4 border-b border-gray-base">
          <div>
            <h1 className="text-xl font-semibold">Who We Are Features</h1>
            <p className="text-sm text-gray-400">
              Manage all Who We Are features
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <Link to="/add-who-we-are-features">
            ...
            {features.length > 0 ? (
              features.map((feature, index) => (
                ...
                >
                  ...
                  <td className="py-1 px-4 sm:py-1.5 sm:px-5 border-l border-b border-gray-base max-w-xs">
                    <p className="truncate">{feature.description}</p>
                  </td>

                  <td className="py-2 px-4 border-l border-b border-gray-base">
                    <div className="flex gap-2">
                      <Link to={`/edit-who-we-are-features/${feature.id}`}>
                        <button
...
export default AllWhoWeAreFeatures;
