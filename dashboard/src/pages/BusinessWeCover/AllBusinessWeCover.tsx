...
} from "../../redux/api/businessWeCover";
...
const AllBusinessWeCover: React.FC = () => {
  ...
  const [deleteBusiness] = useDeleteBusinessWeCoverMutation();

  const businessList: BusinessWeCover[] = data?.data || [];
  const totalPages = data?.meta?.totalPages ?? 1;
  const totalItems = data?.meta?.total ?? 0;

  const handleDeleteBusiness = async (business: BusinessWeCover) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete business "${business.name}"?`,
        icon: "warning",
      ...
      await deleteBusiness(business.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Business "${business.name}" has been deleted.`,
        timer: 1500,
  ...
  return (
    <div className="rounded-lg border border-gray-base bg-black-base overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-4 border-b border-gray-base">
        <div>
          <h1 className="text-xl font-semibold">Business We Cover</h1>
          <p className="text-sm text-gray-400">Manage all Business We Cover</p>
        </div>
      ...
      <table className="w-full min-w-160 sm:min-w-full">
        <thead className="bg-black-solid">
          <tr>
            <th className="py-2 px-4 text-left border-b border-gray-base font-semibold text-sm">
              #
            </th>
            <th className="py-2 px-4 text-left border-l border-b border-gray-base font-semibold text-sm">
              Name
            </th>
            <th className="py-2 px-4 text-left border-l border-b border-gray-base font-semibold text-sm">
              Service
          ...
          {businessList.length > 0 ? (
            businessList.map((business, index) => (
              ...
              >
                ...
                <td className="py-2 px-4 border-l border-b border-gray-base">
                  {business.service?.name || "N/A"}
                </td>
                <td className="py-2 px-4 border-l border-b border-gray-base">
                  <div className="flex gap-2">
                    <Link to={`/edit-business-we-cover/${business.id}`}>
...
export default AllBusinessWeCover;
