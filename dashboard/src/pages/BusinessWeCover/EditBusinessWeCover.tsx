...
interface EditBusinessWeCoverFormValues {
  name: string;
  service_id: string;
}

const EditBusinessWeCover: React.FC = () => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditBusinessWeCoverFormValues>({
    defaultValues: { name: "", service_id: "" },
  });

  // Fetch BusinessWeCover data
  const { data: businessData, isLoading: isFetching } = useGetSingleBusinessWeCoverQuery(id!, {
    skip: !id,
  });
  // Fetch all services for dropdown
  const { data: servicesData, isLoading: isServiceLoading } = useGetAllServicesQuery({ search: "" });
  const services = servicesData?.data || [];
  const [updateBusinessWeCover, { isLoading }] = useUpdateBusinessWeCoverMutation();

  // Populate form and default dropdown value
  useEffect(() => {
  ...
  const onSubmit: SubmitHandler<EditBusinessWeCoverFormValues> = async (formData) => {
    if (!id) {
      Swal.fire("Error!", "Invalid BusinessWeCover ID", "error");
      return;
    }
    try {
      const response = (await updateBusinessWeCover({ id, data: formData }).unwrap());
      if (response.success) {
        toast.success("BusinessWeCover updated successfully!");
        navigate("/business-we-cover");
      } else {
        toast.error(response.message || "Failed to update BusinessWeCover");
      }
    } catch (err: unknown) {
  ...
  if (isFetching) return <p className="text-center mt-10">Loading BusinessWeCover...</p>;

  return (
    <div className={`border bg-black-base border-gray-base rounded-lg  overflow-hidden p-6 mb-6`}>
      <PageHeader
        title="Edit BusinessWeCover"
        breadcrumbs={[
          { title: "Dashboard", link: "/" },
          { title: "BusinessWeCover", link: "/business-we-cover" },
          { title: "Edit BusinessWeCover" },
        ]}
      />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <fieldset className="space-y-5">
          <Input
            label="BusinessWeCover"
            text="name"
            placeholder="Enter BusinessWeCover"
            register={register("name", {
              required: "BusinessWeCover is required",
...
export default EditBusinessWeCover;
