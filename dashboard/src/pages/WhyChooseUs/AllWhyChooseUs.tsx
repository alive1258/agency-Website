...
const AllWhyChooseUs: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, error, isLoading, refetch, isFetching } = useGetAllWhyChooseUsQuery({
    search: debouncedSearch,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteWhyChooseUs] = useDeleteWhyChooseUsMutation();

  const items: WhyChooseUs[] = data?.data || [];
  const totalPages = data?.meta?.totalPages ?? 1;
  const totalItems = data?.meta?.total ?? 0;

  const handleDelete = async (item: WhyChooseUs) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete headline "${item.headline}"?`,
        icon: "warning",
      ...
      await deleteWhyChooseUs(item.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Headline "${item.headline}" has been deleted.`,
        timer: 1500,
    ...
    <div className="rounded-lg border border-gray-base bg-black-base overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-4 border-b border-gray-base">
        <div>
          ...
          <p className="text-sm text-gray-400">Manage all Why Choose Us items</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-160 sm:min-w-full">
          <thead className="bg-black-solid">
            <tr>
              <th className="py-2 px-4 border-b border-gray-base font-semibold text-sm text-left">#</th>
              <th className="py-2 px-4 border-l border-b border-gray-base font-semibold text-sm text-left">Headline</th>
              <th className="py-2 px-4 border-l border-b border-gray-base font-semibold text-sm text-left">Service</th>
              <th className="py-2 px-4 border-l border-b border-gray-base font-semibold text-sm text-left">Actions</th>
            </tr>
          </thead>
          <tbody>
            {items.length > 0 ? (
              items.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-2 px-4 border-l border-b border-gray-base">{item.headline}</td>
                  <td className="py-2 px-4 border-l border-b border-gray-base">{item.service?.name || "-"}</td>
                  <td className="py-2 px-4 border-l border-b border-gray-base">
                    <div className="flex gap-2">
                      <Link to={`/edit-why-choose-us/${item.id}`}>
                        <button className="p-2 cursor-pointer rounded-md transition-colors hover:bg-gray-200 text-green-600 dark:hover:bg-gray-800 dark:text-green-400" title="Edit">
                          <Edit size={16} />
                        </button>
                      </Link>
                      <button
              ...
              <tr>
                <td colSpan={4} className="py-8 text-center">
                  <div className="inline-block bg-black-solid px-6 py-4 rounded-2xl">
                    ...
                    <span className="text-gray-500 text-sm font-medium">No Data Found</span>
                  </div>
                </td>
              </tr>
            )}
